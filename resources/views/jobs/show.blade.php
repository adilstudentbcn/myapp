        ...
        @auth
          <section class="mt-8 pt-6 border-t border-white/10 space-y-4">
            ...
            <form method="POST" action="{{ route('jobs.apply', $job) }}" enctype="multipart/form-data" class="space-y-4">
              ...
              {{-- NEW: CV file upload --}}
              <div>
                <label class="block text-sm font-semibold mb-1" for="cv_file">
                  Upload CV (PDF, DOC, DOCX – optional)
                </label>
                <input id="cv_file" name="cv_file" type="file" accept=".pdf,.doc,.docx" class="block w-full text-sm text-gray-300
